...
class MyWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.ADC = FigureCanvas(self.fig)
        self.DTOF = FigureCanvas(self.fig2)
        self.ABS = FigureCanvas(self.fig3)
        self.VOL = FigureCanvas(self.fig4)
        ...
        wave_info_layout.addWidget(Button_clear_wave)
        ...
        self.Lbl_AbsTOF_Zero_comp_UPS_index = QLabel()
        self.Lbl_AbsTOF_Zero_comp_UPS_index.setText("AbsTOF_Zero_UPS_idx")
        self.Lbl_AbsTOF_Zero_comp_UPS_index.setAlignment(Qt.AlignCenter)
        self.Lbl_AbsTOF_Zero_comp_DNS_index = QLabel()
        self.Lbl_AbsTOF_Zero_comp_DNS_index.setText("AbsTOF_Zero_DNS_idx")
        self.Lbl_AbsTOF_Zero_comp_DNS_index.setAlignment(Qt.AlignCenter)
        ...
        AbsTOF_info_layout2 = QHBoxLayout()
        AbsTOF_info_layout2.addWidget(self.Lbl_AbsTOF_Zero_comp_UPS_index)
        AbsTOF_info_layout2.addWidget(self.Lbl_AbsTOF_Zero_comp_DNS_index)
        
        AbsTOF_layout = QVBoxLayout()
        AbsTOF_layout.addLayout(AbsTOF_info_layout)
        AbsTOF_layout.addLayout(AbsTOF_info_layout2)
        ...
        Volume_info_layout2 = QHBoxLayout()
        Volume_info_layout2.addWidget(self.Lbl_T_Pres_mean)

        
        Volume_layout = QVBoxLayout()
        Volume_layout.addLayout(Volume_info_layout)
        Volume_layout.addLayout(Volume_info_layout2)
        ...
        while(1):
            ...
            if P is None:
                ...
                try:
                    ...
                    self.Lbl_AbsTOF_Zero_comp_UPS_index.setText("UPS_Zero = {0:.3f}".format(((raw_list[1]*1000000000)+40000-(int(self.txt_ADC_Gap.text())*1000))/(1000000/int(self.txt_Sig_freq.text()))))
                    self.Lbl_AbsTOF_Zero_comp_DNS_index.setText("DNS_Zero = {0:.3f}".format(((raw_list[2]*1000000000)+40000-(int(self.txt_ADC_Gap.text())*1000))/(1000000/int(self.txt_Sig_freq.text()))))
                    ...
                    Thread(target = self.wave_dtof(DTOF_buffer)).start()
                    Thread(self.wave_abstof(AbsTOF_UPS_buffer,AbsTOF_DNS_buffer)).start()
                    Thread(self.wave_vol(VOL_buffer,VOL_mean_buffer)).start()
                    ...
                    self.Lbl_Standard_Volume_mean.setText("Standard_Vol = {0:.3f} [l/min]".format(Standard_VOL_mean))
                    ...
                    self.Lbl_T_Pres_mean.setText("Stan_T = {0:.3f},   Thermo  = {1:.1f},   Pressure = {2:}, Accurancy = {3:.3f}%".format(Standard_T_buffer[-1],val_T,val_P,val_Accu*100))
